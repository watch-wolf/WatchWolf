...
%\myworries{There's one downside on using Spigot's Material: it doesn't describes perfectly the block. In some aspects it will, for example, distinguish between wood types, but it won't differentiate between a wooden stair and a wooden stair with water.}

%\myworries{That's why there's some prefixes and suffixes (that will be discussed in the following subsections) surrounding the original Spigot name, to make every possible Minecraft block combination appear in the \hyperref[type:block]{block} enum. Just to clarify, all the block modifiers has also been extracted from Spigot (all \cite{spigot-blockdata}'s subinterfaces).}
...
\myworries{combine with cake if >0}
...
\subsubsection{Eye}\label{spigot-types:eye}
Defaults to false.
...
\subsubsection{Hinge}\label{spigot-types:hinge}
...
\subsubsection{Inverted}\label{spigot-types:inverted}
...
% TODO not ready in this version
...
%\myworries{... wool/glass/bed/candle/banner/GLAZED\_TERRACOTTA}
%
%\begin{longtable}{ |c|c| }
%	\hline
%	Index & Color \\
...
%\myworries{... si head meeans extended}
%
%\begin{table}[H]
%\centering
%\begin{tabular}{ |c|c| }
...
%\myworries{... boolean; log/wood}
%
%\subsubsection{Infested}\label{spigot-types:infested}
%\myworries{... silverfish in block; boolean; stones}
%
%\subsubsection{Dead}\label{spigot-types:dead}
%\myworries{... coral outside water; boolean; coral}
%
%\subsubsection{Heads}
%\myworries{...}
%
...
%\myworries{...; AMETHYST\_BUD \& DRIPLEAF}

\subsection{Material modifiers aggregation}
\myworries{some modifiers are similar, so let's add them together}

\subsubsection{Direction aggregation}\label{spigot-types:direction}
\myworries{modifier, followed by the condition to set the bit to 1}
...
\subsubsection{Parts aggregation}\label{spigot-types:parts}
\myworries{modifier; up to 4b}
...
%\myworries{modifier; up to 16b}
%\myworries{iron and gold pressure plates have their name as 'heavy/light weighted pressure plate'}
%\myworries{MOSS\_CARPET}
%
%\begin{longtable}{ |c|c| }
%\hline
%Property & Value \\
...
%\end{longtable}
%
%\footnotetext{1 is the liquid value of water.}